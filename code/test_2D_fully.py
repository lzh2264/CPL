...
parser.add_argument('--root_path', type=str,
                    default='D:\Code\CPL\data\ACDC', help='Name of Experiment')
    ...
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            if FLAGS.model == "unet_urds":
                out_main, _, _, _, feature = net(input)
            else:
                out_main = net(input)
...
def Inference(FLAGS):
    ...
    snapshot_path = "D:\\Code\\CPL\\model\\{}_{}_labeled_GAI2\\{}".format(
        FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
    test_save_path = "D:\\Code\\CPL\\model\\{}_{}_labeled_GAI2\\{}_predictions\\".format(
        FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
