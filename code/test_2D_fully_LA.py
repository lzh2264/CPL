...
parser.add_argument('--exp', type=str,
                    default='LA/Entropy_Minimization', help='experiment_name')
...
def test_single_volume(case, net, test_save_path, FLAGS):
    h5f = h5py.File(FLAGS.root_path + "/data/test/{}.h5".format(case), 'r')
    image = h5f['image'][:]
    label = h5f['label'][:]
    ...
    for ind in range(image.shape[0]):
        ...
        slice = zoom(slice, (112 / x, 112 / y), order=0)
...
def Inference(FLAGS):
    ...
    snapshot_path = "D:\\Code\\CPL\\model\\{}_{}_labeled\\{}".format(
        FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
    test_save_path = "D:\\Code\\CPL\\model\\{}_{}_labeled\\{}_predictions\\".format(
        FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
